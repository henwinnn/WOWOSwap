...
export default function TokenSelectorModal({ isOpen, onClose, onSelect, tokens }: TokenSelectorModalProps) {
  ...
  const filteredTokens = tokens.filter((token) => {
    ...
    if (activeFilter === "stables") {
      return matchesSearch && ["USDC", "USDT", "DAI", "EURC", "IDRX"].includes(token.symbol)
    }
  ...
  return (
    <UIModal isOpen={isOpen} onClose={onClose} title="Select Token" className="max-w-lg">
      {/* Search Input */}
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm">
                  {token.address.substring(0, 6)}...{token.address.substring(token.address.length - 5)}
                </p>
              </div>
            </div>
