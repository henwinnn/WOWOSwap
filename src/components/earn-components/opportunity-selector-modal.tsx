...
export default function OpportunitySelectorModal({ isOpen, onClose, onSelect, vaults }: OpportunitySelectorModalProps) {
  const [activeFilter, setActiveFilter] = useState("all")
  const [activeNetwork, setActiveNetwork] = useState("all")
  const [sortByAPY, setSortByAPY] = useState(true)

  // Sort vaults by APY (descending)
  const sortedVaults = [...vaults].sort((a, b) => (sortByAPY ? b.apy - a.apy : a.apy - b.apy))
    ...
    <UIModal isOpen={isOpen} onClose={onClose} title="Select Opportunity">
      ...
      <div className="flex justify-between items-center px-4 py-2 border-b border-gray-100">
        ...
        <div className="flex items-center">
          <button className="text-gray-500 text-sm flex items-center" onClick={() => setSortByAPY(!sortByAPY)}>
            APY
            <ArrowDown className={`ml-1 h-4 w-4 transition-transform ${sortByAPY ? "" : "rotate-180"}`} />
          </button>
          ...
          >
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-blue-500 text-sm">+ ${vault.yearlyReward.toFixed(2)} over 1y</p>
              </div>
            </div>
            <div className="flex items-center">
              <p className="font-bold text-gray-900 text-xl">{vault.apy.toFixed(2)}%</p>
